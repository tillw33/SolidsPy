...
def readin(folder=""):
    ...
    nodes = np.loadtxt(os.path.join(folder,'nodes.txt'), ndmin=2)
    mats = np.loadtxt(os.path.join(folder, 'mater.txt'), ndmin=2)
    elements = np.loadtxt(os.path.join(folder,'eles.txt'), ndmin=2,
                          dtype=np.int)
    loads = np.loadtxt(os.path.join(folder,'loads.txt'), ndmin=2)
    return nodes, mats, elements, loads


def echomod(nodes, mats, elements, loads, folder=""):
    """Create echoes of the model input files"""
    np.savetxt(os.path.join(folder, "KNODES.txt"),
               nodes, fmt='%5.2f', delimiter=' ')
    np.savetxt(os.path.join(folder, "KMATES.txt"),
                            mats, fmt='%5.2f', delimiter=' ')
    np.savetxt(os.path.join(folder, "KELEMS.txt")
               , elements, fmt='%d', delimiter=' ')
    np.savetxt(os.path.join(folder, "KLOADS.txt"),
               loads, fmt='%5.2f', delimiter=' ')
